...
from saleapp.model import Category, Product, User, Receipt, ReceiptDetail, UserRole
...
def load_categories():
    return Category.query.all() #Lấy từ CSDL
...
def load_products(cate_id=None, kw=None, from_price=None, to_price=None, page=1):
    ...
    if cate_id:
        ...
        #Kiểm tra những sản phẩm có trùng id do người dùng truyền vào
    if kw:
        products = products.filter(Product.name.contains(kw))
        # Kiểm tra những sản phẩm có chưa trong tên sản phẩm do người dùng truyền vào

    if from_price:
        products = products.filter(Product.from_price.__ge__(from_price))

    if to_price:
        products = products.filter(Product.to_price.__le__(to_price))
...
def check_login(username, password, role=UserRole.USER):
    if username and password:
        ...
        #So sánh username và password dưới CSDL với username và password vừa nhập
